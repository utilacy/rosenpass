...
mod tests {
    ...
    use serde_json;
    use std::sync::Once;
    use crate::secret_policy_try_use_memfd_secrets;
    ...
    static INIT_SECRET_POLICY: Once = Once::new();

    pub fn test_init_secret_memory_policy() {
        INIT_SECRET_POLICY.call_once(|| {
            #[cfg(target_os = "linux")]
            {
    ...
    fn secret_roundtrip_json() {
        ...
        assert_eq!(original.secret(), recovered.secret(), "Secret bytes must match after roundtrip");
    }
    ...
    fn public_roundtrip_json() {
        ...
        assert_eq!(json, json2, "Public must serialize identically after roundtrip");
    }
    ...
    fn public_box_roundtrip_json() {
        ...
        // Prefer constructor if available.
        // If `PublicBox::<N>::new` is not available in your API, replace with:
        // let inner = Box::new(Public::<N>::from_slice(&src));
        // let original = PublicBox { inner };
        let original = PublicBox::<N>::new(src);

        let json = serde_json::to_string(&original).expect("serialize");
        let recovered: PublicBox<N> = serde_json::from_str(&json).expect("deserialize");
        let json2 = serde_json::to_string(&recovered).expect("re-serialize");
        assert_eq!(json, json2, "PublicBox must serialize identically after roundtrip");
    }
    ...
    fn secret_len_mismatch_is_error() {
        ...
        assert!(res.is_err(), "Deserializing into a larger fixed size must error");
    }
    ...
    fn public_box_len_mismatch_is_error() {
        ...
        assert!(res.is_err(), "Deserializing into a different fixed size must error");
    }
}
